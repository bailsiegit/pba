...
{
	//$volunteerQuery = "SELECT volunteers.Role, volunteers.VolId, members.MemberId, members.FirstName, members.LastName FROM members	 
	//INNER JOIN volunteers ON members.MemberId = volunteers.MembId WHERE volunteers.Year = $getyear";
	$volunteerQuery = "SELECT volunteers.Role, volunteers.VolId, members.MemberId, members.FirstName, members.LastName FROM members	 
	INNER JOIN volunteers ON members.MemberId = volunteers.MembId WHERE volunteers.Year = ?";
...
{
	//$volunteerQuery = "SELECT years.YearText, volunteers.Role, volunteers.VolId, members.MemberId, members.FirstName, members.LastName FROM ((members	 
	//INNER JOIN volunteers ON members.MemberId = volunteers.MembId)
	//INNER JOIN years ON volunteers.Year = years.YearId) WHERE volunteers.Role = '$getrole' ORDER BY years.YearId DESC";
	$volunteerQuery = "SELECT years.YearText, volunteers.Role, volunteers.VolId, members.MemberId, members.FirstName, members.LastName FROM ((members	 
	INNER JOIN volunteers ON members.MemberId = volunteers.MembId)
	INNER JOIN years ON volunteers.Year = years.YearId) WHERE volunteers.Role = ? ORDER BY years.YearId DESC";
	$stmt = mysqli_prepare($link, $volunteerQuery);
...
//$volunteerResult = mysqli_query($link, $volunteerQuery);
if (!$volunteerResult) 
{
