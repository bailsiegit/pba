<?php
//Rev 1 19/11/2025
//This page is reached from a button on awards, accolades, volunteer and employee pages in the activity area
...
{
	//save all the POST values into same variables as GET to refresh page after upload they are then put in hidden inputs to be returned via POST
	...
	$root = realpath(__DIR__ . '/../documents');
	$uploadDir = $root . "/$getactivity/";
...
echo '<h2 style="text-align:center; margin:5px; width:100%;">'.$type[$getactivity].' Documents - '.$yrtxt.' - '.$actname.'</h2>';
...
if($_SESSION['accesslevel'] > 2) //only show upload dialogue if user has permission
{
	...
	<form action="pbainddocslist.php" method="post" enctype="multipart/form-data">
