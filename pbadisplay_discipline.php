...
if (mysqli_num_rows($incidentResult) > 0) 
{
	...
	{
		$expdate = empty($incident['ExpiryDate']) ? "" : date("d/m/Y", strtotime($incident['ExpiryDate']));
		if($_SESSION['accesslevel'] > 3)
		{
			echo '<tr><td><a href="pbaperson.php?pid='.$incident['MemberId'].'">'.$incident['FirstName'].' '.$incident['LastName'].'</a></td>
				...
				<td>'.$expdate.'</td>
				<td><a class="buttonlink" href="pbainddocslist.php?actid=tb&yid='.$incident['YearId'].'refid='.$incident['IncidentId'].'">Docs</a></td>
