...
//this page lists all the memberships the person has held
...
{
	echo '<p><br>No record of '.$person['FirstName'].' being a member.<br><br></p>';
}
else
{
	echo '<p> </p><table width="90%">';
		
	echo '<tr><th width="15%">Year</th><th width="30%">Membership Type</th><th width="15%">Start Date</th><th>End Date</th></tr>';
	while($row = mysqli_fetch_array($r, MYSQLI_ASSOC))
	{
		if($row['start'] != '0000-00-00'){ $startdate = date("d/m/y",strtotime($row['start']));}else{$startdate = '';}
		if($row['end'] != '0000-00-00'){ $enddate = date("d/m/y", strtotime($row['end']));}else{$enddate = '';}
		echo '<tr><td>'.$row['YearText'].'</td><td>' . $row['Type'] . '</td>
