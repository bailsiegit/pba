<?php
//Rev 1 19/11/2025
//this page is called either directly or via javascript by the activity employees page
//this page creates the table for display on that page
...
//$qyear = "SELECT YearText FROM years WHERE YearId = $getyear";
...
if($getrole < 1)//if $getrole is zero then display for selected year
{
	...
	$employeeQuery = "SELECT employees.Role, employees.EmpId, members.MemberId, members.FirstName, members.LastName FROM members	 
	INNER JOIN employees ON members.MemberId = employees.MembId WHERE employees.Year = ?";
...
else //if $getrole has something other than zero, display for selected role
{
	...
	$employeeQuery = "SELECT years.YearText, employees.Role, employees.EmpId, members.MemberId, members.FirstName, members.LastName FROM ((members	 
	INNER JOIN employees ON members.MemberId = employees.MembId)
	INNER JOIN years ON employees.Year = years.YearId) WHERE employees.Role = ? ORDER BY years.YearId DESC";
	$stmt = mysqli_prepare($link, $employeeQuery);
	mysqli_stmt_bind_param($stmt, "s", $getrole);
	echo '<h4>'.$getrole.' - Employees</h4>';
}
	
mysqli_stmt_execute($stmt);
$employeeResult = mysqli_stmt_get_result($stmt);
//$employeeResult = mysqli_query($link, $employeeQuery);
if (!$employeeResult) 
{
...
if (mysqli_num_rows($employeeResult) > 0) 
{
	...
	elseif($_SESSION['accesslevel'] > 3 && $getrole < 1)
	{
	...
	while ($employee = mysqli_fetch_assoc($employeeResult)) 
	{
		if($getrole > 1)
		{
			echo '<tr><td>'.$employee['YearText'].'</td>
				<td><a href="pbaperson.php?pid='.$employee['MemberId'].'">'.$employee['FirstName'].' '.$employee['LastName'].'</a></td>
				<td>'.$employee['Role'].'</td></tr>';
		}
		elseif($_SESSION['accesslevel'] > 3 && $getrole < 1)
		{
			echo '<tr><td><a href="pbaperson.php?pid='.$employee['MemberId'].'">'.$employee['FirstName'].' '.$employee['LastName'].'</a></td>
				<td>'.$employee['Role'].'</td>
				<td><a onclick="return confirm(\'Are you sure?\');" class="buttonlink" href="pbadeleterecords.php?vid='.$employee['EmpId'].'&yid='.$getyear.'">Delete</a></td></tr>';
		}
		else
		{
			echo '<tr><td><a href="pbaperson.php?pid='.$employee['MemberId'].'">'.$employee['FirstName'].' '.$employee['LastName'].'</a></td>
...
{
	echo '<option value="">No employees found in '.$yeartext['YearText'].'</option><br><br>';
}
echo '</table>';
